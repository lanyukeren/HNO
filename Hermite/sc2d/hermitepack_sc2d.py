...
def norm_Hermite_func_Vand_expr(Nx, modes) -> (torch.Tensor, torch.Tensor, torch.Tensor):
    ...
    x_hg_tensor = torch.from_numpy(x_hg).float()
    w_hg_tensor = torch.from_numpy(w_hg).float()
    Vandmat_tensor = torch.from_numpy(Vandmat).float()
...
class Vandermode_transform2d:
    ...
    def to(self, device):
        ...
        self.forw_mat = self.forw_mat.float().to(device)
        self.back_mat = self.back_mat.float().to(device)
